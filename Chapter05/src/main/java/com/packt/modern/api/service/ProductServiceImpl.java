...
public class ProductServiceImpl implements ProductService {
  ...
  public Flux<ProductEntity> getAllProducts() {
    return repository.findAll()
        .flatMap(products ->
            Mono.just(products)
                .zipWith(tagRepo.findTagsByProductId(products.getId()).collectList())
  ...
  public Mono<ProductEntity> getProduct(String id) {
    ...
    Flux<TagEntity> tags = tagRepo.findTagsByProductId(UUID.fromString(id)).subscribeOn(Schedulers.boundedElastic());
